...
class SemanticAnalyzer:
    ...
    def visit_program_node(self, node):
        ...
        for s in node.stdlibs:
            print("CHECKING STDLIBS!!!!!!")
            if s not in ['Cmath', 'Cstring', 'Carray']:
                self.logError("Invalid import. Must be a standard library ('Cmath', 'Cstring', 'Carray').", node.err_t)
            ...
            case '%':
                if dtype[1] in ['float', 'double'] or right_type[1] in ['float', 'double']:
                    ...
                    if left_type[1] in self.numtypes and right_type[1] in self.numtypes:
                        ...
                        self.logError(f"Type mismatch for arithmetic expression, expected numeric value (int, long) for both operands, but got {left_type[1]} and {right_type[1]}.", left_err)
